...
export default async function handler(req, res) {
  ...
  try {
    ...
    const minimal = courses.map(c => ({ id: c.id, name: c.name, course_code: c.course_code, workflow_state: c.workflow_state }));
    return res.status(200).json({ ok: true, baseUrl, count: minimal.length, courses: minimal });
  } catch (e) {
    console.error('debug/canvas/courses error', e);
